...
func syncServiceCatalogAPIServer_v311_00_to_latest(c ServiceCatalogAPIServerOperator, originalOperatorConfig *operatorv1.ServiceCatalogAPIServer) (bool, error) {
	...
	directResourceResults := resourceapply.ApplyDirectly(c.kubeClient, c.eventRecorder, v311_00_assets.Asset, "v3.11.0/openshift-svcat-apiserver/ns.yaml", "v3.11.0/openshift-svcat-apiserver/svc.yaml", "v3.11.0/openshift-svcat-apiserver/sa.yaml", "v3.11.0/openshift-svcat-apiserver/cr-aggregate-to-admin.yaml", "v3.11.0/openshift-svcat-apiserver/cr-aggregate-to-edit.yaml", "v3.11.0/openshift-svcat-apiserver/cr-aggregate-to-view.yaml", "v3.11.0/openshift-svcat-apiserver/crb-auth-delegator-binding.yaml", "v3.11.0/openshift-svcat-apiserver/cr-namespace-viewer.yaml", "v3.11.0/openshift-svcat-apiserver/crb-namespace-viewer-binding.yaml", "v3.11.0/openshift-svcat-apiserver/cr-readiness-probe.yaml", "v3.11.0/openshift-svcat-apiserver/crb-readiness-binding.yaml", "v3.11.0/openshift-svcat-apiserver/cr-sar-creator.yaml", "v3.11.0/openshift-svcat-apiserver/crb-sar-creator-binding.yaml", "v3.11.0/openshift-svcat-apiserver/cr-serviceclass-viewer.yaml", "v3.11.0/openshift-svcat-apiserver/crb-serviceclass-viewer-binding.yaml", "v3.11.0/openshift-svcat-apiserver/rolebinding-extension-apiserver-auth-reader.yaml", "v3.11.0/openshift-svcat-apiserver/role-extension-apiserver-auth-reader.yaml")
	...
	case len(availableConditionMessages) == 1:
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: operatorv1.OperatorStatusTypeAvailable, Status: operatorv1.ConditionFalse, Reason: availableConditionReason, Message: availableConditionMessages[0]})
	case len(availableConditionMessages) > 1:
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: operatorv1.OperatorStatusTypeAvailable, Status: operatorv1.ConditionFalse, Reason: "Multiple", Message: strings.Join(availableConditionMessages, "\n")})
	default:
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: operatorv1.OperatorStatusTypeAvailable, Status: operatorv1.ConditionTrue})
	}
	...
	if len(progressingMessages) == 0 {
		...
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: operatorv1.OperatorStatusTypeProgressing, Status: operatorv1.ConditionTrue, Reason: "DesiredStateNotYetAchieved", Message: strings.Join(progressingMessages, "\n")})
	}
	...
	if len(errors) > 0 {
		...
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: workloadDegradedCondition, Status: operatorv1.ConditionTrue, Message: message, Reason: "SyncError"})
	} else {
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: workloadDegradedCondition, Status: operatorv1.ConditionFalse})
	}
	if v1helpers.IsOperatorConditionTrue(operatorConfig.Status.Conditions, operatorv1.OperatorStatusTypeAvailable) {
		operandVersion := status.VersionForOperand(operatorclient.OperatorNamespace, actualDaemonSet.Spec.Template.Spec.Containers[0].Image, c.kubeClient.CoreV1(), c.eventRecorder)
...
func manageAPIServices_v311_00_to_latest(client apiregistrationv1client.APIServicesGetter) ([]*apiregistrationv1.APIService, error) {
	...
	for _, apiServiceGroupVersion := range apiServiceGroupVersions {
		obj := &apiregistrationv1.APIService{ObjectMeta: metav1.ObjectMeta{Name: apiServiceGroupVersion.Version + "." + apiServiceGroupVersion.Group, Annotations: map[string]string{"service.alpha.openshift.io/inject-cabundle": "true"}}, Spec: apiregistrationv1.APIServiceSpec{Group: apiServiceGroupVersion.Group, Version: apiServiceGroupVersion.Version, Service: &apiregistrationv1.ServiceReference{Namespace: operatorclient.TargetNamespaceName, Name: "api"}, GroupPriorityMinimum: 9900, VersionPriority: 15}}
