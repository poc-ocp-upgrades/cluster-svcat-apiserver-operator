...
func RunOperator(ctx *controllercmd.ControllerContext) error {
	...
	v1helpers.EnsureOperatorConfigExists(dynamicClient, v311_00_assets.MustAsset("v3.11.0/openshift-svcat-apiserver/operator-config.yaml"), schema.GroupVersionResource{Group: operatorv1.GroupName, Version: operatorv1.GroupVersion.Version, Resource: "servicecatalogapiservers"})
	operatorConfigInformers := operatorv1informers.NewSharedInformerFactory(operatorConfigClient, 10*time.Minute)
	kubeInformersForNamespaces := v1helpers.NewKubeInformersForNamespaces(kubeClient, "", operatorclient.UserSpecifiedGlobalConfigNamespace, operatorclient.MachineSpecifiedGlobalConfigNamespace, operatorclient.KubeAPIServerNamespaceName, operatorclient.OperatorNamespace, operatorclient.TargetNamespaceName, "kube-system")
	...
	workloadController := workloadcontroller.NewWorkloadController(os.Getenv("IMAGE"), versionRecorder, operatorConfigInformers.Operator().V1().ServiceCatalogAPIServers(), kubeInformersForNamespaces.InformersFor(operatorclient.TargetNamespaceName), kubeInformersForNamespaces.InformersFor(operatorclient.EtcdNamespaceName), kubeInformersForNamespaces.InformersFor(operatorclient.KubeAPIServerNamespaceName), kubeInformersForNamespaces.InformersFor(operatorclient.UserSpecifiedGlobalConfigNamespace), apiregistrationInformers, configInformers, operatorConfigClient.OperatorV1(), configClient.ConfigV1(), kubeClient, apiregistrationv1Client.ApiregistrationV1(), ctx.EventRecorder)
	...
	clusterOperatorStatus := status.NewClusterOperatorStatusController("service-catalog-apiserver", append([]configv1.ObjectReference{{Resource: "namespaces", Name: operatorclient.UserSpecifiedGlobalConfigNamespace}, {Resource: "namespaces", Name: operatorclient.MachineSpecifiedGlobalConfigNamespace}, {Resource: "namespaces", Name: operatorclient.OperatorNamespace}, {Resource: "namespaces", Name: operatorclient.TargetNamespaceName}}, workloadcontroller.APIServiceReferences()...), configClient.ConfigV1(), configInformers.Config().V1().ClusterOperators(), operatorClient, versionRecorder, ctx.EventRecorder)
